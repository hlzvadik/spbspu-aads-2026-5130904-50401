...
BOOST_AUTO_TEST_CASE(begin_test)
{
  ...
  BOOST_CHECK(b.begin() == goltsov::detail::makeLCIterByPtr< int >(nullptr));
...
BOOST_AUTO_TEST_CASE(end_test)
{
  ...
  const goltsov::List< int > b;
  BOOST_CHECK(b.end() == goltsov::detail::makeLCIterByPtr< int >(nullptr));
}
...
{
  goltsov::List< int > a;
  BOOST_CHECK((* a.insert(a.begin(), 1)) == 1 && a.begin() != goltsov::detail::makeLIterByPtr< int >(nullptr));
  BOOST_CHECK((* a.insert(a.begin(), 2)) == 2);
  int c1 = 1;
  int c2 = 2;
  a.clear();
  BOOST_CHECK((* a.insert(a.begin(), c1)) == 1 && a.begin() != goltsov::detail::makeLIterByPtr< int >(nullptr));
